...
{
  control_msgs::JointTrajectoryActionResult result;
  ...
  if(planner.solve(planning_response))
  {
    trajectory_msgs::JointTrajectory robot_traj;
    tesseract_rosutils::toMsg(robot_traj, planning_response.joint_trajectory);
    if (!this->motion_streaming(robot_traj))
    {
      ROS_ERROR("Motion Failed");
      joint_value_server_.setAborted();
      return;
    }
    ROS_INFO("Planning and Motion Execution Succeeded");
...
bool FreespaceSrvExample::motion_streaming(trajectory_msgs::JointTrajectory robot_traj)
{
  //////////////////////////////
